...
def create_filename(yt, extension = FileExtension.MP3.value):
...
def download_video_stream(yt, filename, temp_dir):
...
def download_audio_stream(yt, filename, temp_dir):
...
def download_audio_mp3_from_youtube(url):
    ...
    try:
        ...
        filename = create_filename(yt)
        temp_dir = Config.TEMPORARY_DIRECTORY
        
        temp_file_path = download_audio_stream(yt, filename, temp_dir)
...
def download_video_mp4_from_youtube(url):
    ...
    try:
        yt = get_youtube_video(url)
        filename = create_filename(yt, extension=FileExtension.MP4.value)
        temp_dir = Config.TEMPORARY_DIRECTORY

        temp_file_path = download_video_stream(yt, filename, temp_dir)
        logging.info(f"{Messages.DOWNLOAD_SUCCESSFULL.value} {filename}")
